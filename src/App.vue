...
export default {
  ...
  methods: {
    ...
    async getChampions() {
      try {
        const resChamps = await axios.post(
          'http://localhost:4000/graphql', {
            query: `{
              getChampions {
                name
          ...
          this.champions = resChamps.data.data
      } catch (error) {
        console.error(error)
      }
    },
    ...
    async getChampionByName () {
      try {
        const resChamp = await axios.post('http://localhost:4000/graphql', {
          query: `
        ...
        this.champion = resChamp.data.data.getChampionByName
      } catch (error) {
        console.error(error)
      }
    },
    ...
    async updateAttackDamage () {
      try {
        const resUpdate = await axios.post('http://localhost:4000/graphql', {
          query: `
        ...
        this.updatedChampion = resUpdate.data.data.updateAttackDamage
      } catch (error) {
        console.error(error)
      }
    }
